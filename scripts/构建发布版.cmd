...
"%PYTHON%" -m PyInstaller --noconfirm --clean --windowed --onefile --name GuoLingZhuShou-1.1.1 --paths "%PROJECT_DIR%\src" --distpath release --workpath build\pyinstaller --specpath build --add-data "%PROJECT_DIR%\src\guoling_task_ocr\data;guoling_task_ocr\data" --add-data "%PROJECT_DIR%\.venv\Lib\site-packages\Cython\Utility\CppSupport.cpp;Cython\Utility" --collect-all paddle --collect-all paddleocr --collect-all windows_capture --collect-data Cython "%PROJECT_DIR%\scripts\pyinstaller_entry.py"
if errorlevel 1 goto :failed

echo.
echo Build completed: release\GuoLingZhuShou-1.1.1.exe
pause
exit /b 0
